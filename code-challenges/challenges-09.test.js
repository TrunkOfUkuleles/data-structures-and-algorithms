...
function transformToLis(obj){
  // Solution code here...
  let result = []
  let base = Object.entries(obj)
  base.map((el)=> {result.push(`<li>${el[0]}: ${el[1]}</li>`)})
  return result
}
...
const addPurchases = (arr) => {
  // Solution code here...
  return arr.reduce((acc, cur)=>{return acc + cur.purchasePrice},0)
};
...
const countNumberOfElements = (arr) => {
  // Solution code here...
  return arr.reduce((acc) => {return acc+1} , 0)
};
...
const returnNames = (arr) => {
  // Solution code here...
  return arr.reduce((acc,curr) =>{ acc.push(curr.name)
    return acc},[])
};
...
const countNumberOfChildren = (arr) => {
  ...
  return arr.reduce((acc, curr) => curr.children ?  acc + curr.children.length : acc , 0)
};
...
const extractStat = (statName, arr) => {
  // Solution code here...
  return arr.reduce((acc,curr) => curr.stat.name === statName ? curr : acc, {})
};
...
const extractChildren = (arr) => {
  // Solution code here...
  // return arr.slice().filter((el) => el.name.includes('a')).reduce((acc, curr) => {acc.push(curr.name)}, []);

  return arr.slice().filter((el)=> el.name.includes('a')).reduce((acc, curr) => curr.children ?   [...acc, ...curr.children] : acc , [])
};
