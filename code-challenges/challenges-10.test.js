...
const getNames = (arr) => {
  // Solution code here...
  let result =[]
  let names = arr.reduce((a,c)=>{return [...a, c.name]},[])
  console.log(names)
  return names.map( el=> el.split('').reverse().join(''))
};
...
const count = (target, input) => {
  // Solution code here...
  let count = 0
  input.map(el=>{el.map(ell=>{ell===target? count++ : ''})})
  return count
};
...
const totalSum = (input) => {
  // Solution code here...
  let arrrs = input.reduce((a,c)=>{return [...a, ...c]},[])
  return arrrs.reduce((arr,curr)=>{return Number(arr)+Number(curr)},0)
...
const divisibleByFiveTwoToThePower = (input) => {
  // Solution code here...
  function rule(a) {return typeof(a)==='number' && a%5===0}
  let no5 = input.map(el =>  el.filter(rule))

  let result = []

  
  for (let i=0;i<no5.length;i++){
    result.push(no5[i].map(el=>Math.pow(2,el)))
  }
...
let findMaleAndFemale = (data) => {
  // Solution code here...
  let result=[]
  data.map(el=>{if (el.gender==='male'||el.gender==='female') {result.push(el.name) }})
  console.log(result)
...
let findShortest = (data) => {
  // Solution code here...
  let track = {height: '0'}
  data.forEach(el=> {el.height>track.height ? track={...el} : ''})
  return track.name;
};
