...
const toLastNames = people => {
  // Solution code here...
  return people.map(el=> { return el.firstName + " " + el.lastName})
...
const validateWord = (word) => {
  // Solution code here...
  let rogg = /^[A-Za-z]{5,10}$/gm
  return rogg.test(word)
};
...
const hasNumber = (string) => {
  // Solution code here...
  let rygg = /[A-Za-z]+\d/gm
  return rygg.test(string)
};
...
const validateEmail = (email) => {
  // Solution code here...
  let rugg = /^[A-Za-z0-9]+\.?[\w]+?\@[\w]+((\.net$)|(\.org$)|(\.com))$/gm
  return rugg.test(email)
};
...
const validatePhoneNumber = (phoneNumber) => {
  // Solution code here...
  let regg = /^((\(\d{3}\))|(\d{3}))[\s-]?\d{3}[\s-]?\d{4}$/gm 
  // let reagg = /[-.(]*(\d{3})[-.)]*(\d{3})[-.]*(\d{4})$/gm 
  return regg.test(phoneNumber)
};
...
const findTagNames = elements => {
  ...
  let res = []
  let rag = /<+(\w+)/gm
  let test =  elements.filter(el=> rag.exec(el))
  return test[1]
};
