...
const filterStringsWithVowels = (arr) => {
  // Solution code here...
  let re = RegExp('[a|e|i|o|u|A|E|I|O|U]')
  return arr.filter(word => re.test(word))
};  
...
const notInFirstArray = (forbiddenValues, arr) => {
  // Solution code here...
  let re = RegExp(`[${forbiddenValues}.join(' | ')]`)
  if (forbiddenValues.some(el => typeof(el)==='string')){ re = RegExp(`[${forbiddenValues}.split(',').join(' | ')]`)}
  if (arr.length===0){return arr}
  return arr.filter(num => !re.test(num))
};
...
const getBaseStatGreaterThan = (arr, minBaseStat) => {
  ...
  return arr.filter(move => move.baseStat>minBaseStat ? true : '' )
};
...
const getStatName = (arr, minBaseStat) => {
  // Solution code here...
  let rest = getBaseStatGreaterThan(arr)
  return rest.filter(move => move.stat.name)
};
...
const getCharactersWithoutChildren = (arr) => {
  // Solution code here...
  return arr.filter(par => par.children ? '' : par.name)
};
...
const evenOddNumericValues = (arr) => {
  // Solution code here...
  return arr.filter(nums => typeof(nums)==='number' ? true : false).map(numm => numm%2===0 ? 'even' : 'odd')
};
